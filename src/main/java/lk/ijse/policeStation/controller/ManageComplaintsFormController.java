...
import lk.ijse.policeStation.Bo.Custom.Impl.ComplaintBoImpl;
...
import lk.ijse.policeStation.Dao.custom.Impl.OfficerDaoImpl;
...
public class ManageComplaintsFormController {
    ...
    private JFXTextField TxtWitnessInformation;
     ...
     ComplaintBo complaintBo= new ComplaintBoImpl();
    ...
    void BtnDeleteOnAction(ActionEvent event) {
        ...
        if (pressedButton.equals(ButtonType.YES)) {
            try {
                boolean isDeleted = complaintBo.deleteComplaint(id);
                if (isDeleted) {
        ...
        if (validateComplaints()) {
            ...
            try {
                ...
                if (isCitizenIdValid(complaintDto.getCitizenId())) {
                    boolean isSuccess = complaintBo.saveComplaint(complaintDto);
                    if (isSuccess) {
    ...
    void BtnSearchOnAction(ActionEvent event) {
        ...
        try {
            Optional<ComplaintDto> complaint = complaintBo.searchComplaint(code);
    ...
    void BtnUpdateOnAction(ActionEvent event) {
        if (validateComplaints()) {
            ...
            try {
                boolean isUpdated = complaintBo.updateComplaint(updatedComplaintDto);
    ...
    private void  LoardCitizen() throws SQLException, ClassNotFoundException {
        ...
        CitizenDaoImpl citizenDAO = new CitizenDaoImpl(); // Create an instance
        List<String> cusId = citizenDAO.getCitizenIds(); // Call the method on the instance
        TextFields.bindAutoCompletion(TxtCitizenId, cusId);
}

    private void LoardOfficers() throws SQLException, ClassNotFoundException {
        OfficerDaoImpl officerDao=new OfficerDaoImpl();
        List<String> officerIds = officerDao.getOfficerIds();
